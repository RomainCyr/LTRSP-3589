...
2. **CheckIGP: Verify the IGP configuration using Genie Diff.**
   ...
   3. **Compare the collected IGP configuration against the expected one.** 
   4. **Pass or Fail the test depending on the IGP configuration.**
...
- The next 6 bytes (12 digits) represent the system ID. This is unique to each router within the IS-IS domain. It is equivalent to the host or address portion on an IP address.
...
The role of the device are defined in the testbed and can be access with `device.role`.
...
The `exclude` parameter is a list of string to ignore, it can be a regex if it is put in parentheses.
